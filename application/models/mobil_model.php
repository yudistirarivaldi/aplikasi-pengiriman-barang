...
class Mobil_Model extends CI_Model
{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(id_mobil) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(plat) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(jenis) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	{
		$where = "WHERE ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' and plat = '".$this->db->escape_str(strtolower($name))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
